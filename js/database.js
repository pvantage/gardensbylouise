...
//}
    ...
    function populateDB(tx) {
		...
		tx.executeSql('CREATE TABLE IF NOT EXISTS userlogin (id integer primary key autoincrement, user_id integer, lastlogin_time datetime, loggedin integer, fname text, lname text, user_type text, remberme text, uname text, pwd text)');
		 ...
		 tx.executeSql('CREATE TABLE IF NOT EXISTS job_timesheets (id integer primary key autoincrement, job_id integer, customer_id integer, user_id integer, job_date date, from_address text, address_to text, address text, customer_name text, time_type text, start_time time, end_time time, work_time time, mileage float, reimburse float, job_time_id integer DEFAULT "0", lastone integer DEFAULT "0")');
